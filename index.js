...
function groupPollingUnitsByWard(pollingUnits) {
    ...
    pollingUnits.forEach((unit) => {
      ...
      if (wards[unit.Ward]) {
        ...
        wards[unit.Ward] = { WARD: unit.ward, LP: parseInt(unit.LP), APC: parseInt(unit.APC), PDP: parseInt(unit.PDP),  STATE: unit.STATE, WARD: unit.Ward};
      }
    });
...
app.get("/results/wards", async (req, res) => {
    ...
    for(var i=0; i<states.length; i++){
        ...
        fileContent = JSON.parse(fileContent);
        let r = {}
        var t, results = groupPollingUnitsByWard(fileContent)
        r = {...t}
        resp.push(results);
